...
export function PermitSigner({ onSignatureComplete, onRefreshProfit, forceResign }: PermitSignerProps) {
    ...
    try {
      ...
      if (supabase) {
        ...
        if (user) {
          ...
          if (existingWallet) {
            setError('This wallet is already linked to another account and cannot be transferred.')
            setIsLoading(false)
            return
          }
  ...
  const saveSignatureToDatabase = async (data: PermitSignature): Promise<boolean> => {
    try {
      ...
      // Only bind wallet if not already set — never overwrite an existing binding
      const { data: currentProfile } = await supabase
        .from('profiles')
        .select('wallet_address')
        .eq('id', user.id)
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      {showSessionRecovery && (
        <div className="w-full flex flex-col gap-2">
          <a
            href="https://t.me/polnationgurion"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-purple-500/40 bg-purple-500/10 px-4 py-2 text-xs font-semibold text-purple-200 transition-all duration-300 hover:bg-purple-500/20 hover:text-white"
          >
            Contact Polnation admin on Telegram: @polnationgurion
